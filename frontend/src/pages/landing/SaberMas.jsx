// src/pages/landing/SaberMas.jsx
...
const resultadoFortalezas = [
  { emoji: '🎨', label: 'Creatividad',           tag: 'Alta',  color: 'bg-green-100 text-green-700' },
  { emoji: '🧠', label: 'Pensamiento Analítico', tag: 'Alta',  color: 'bg-green-100 text-green-700' },
  { emoji: '💬', label: 'Comunicación',           tag: 'Media', color: 'bg-yellow-100 text-yellow-700' },
  { emoji: '⭐', label: 'Liderazgo',             tag: 'Media', color: 'bg-yellow-100 text-yellow-700' },
  { emoji: '🤝', label: 'Trabajo en Equipo',      tag: 'Media', color: 'bg-yellow-100 text-yellow-700' },
];
...
export default function SaberMas() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* ── Hero ── */}
      <section className="pt-28 pb-16 px-6 bg-gradient-to-b from-green-50 to-white">
        <div className="max-w-6xl mx-auto flex flex-col lg:flex-row items-center gap-10">
          <div className="flex-1">
            <button
              onClick={() => navigate('/servicios')}
              className="flex items-center gap-1 text-sm text-green-600 font-medium mb-6 hover:underline"
            >
              ← Volver a servicios
            </button>
          ...
          {/* Ilustración emoji */}
          <div className="flex-shrink-0 w-48 h-48 bg-green-50 rounded-3xl flex items-center justify-center text-8xl shadow-sm">
            📋
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-6 max-w-6xl mx-auto">
        <h2 className="text-xl font-bold text-gray-800 mb-10">¿Cómo funciona nuestro test vocacional?</h2>
          ...
          {pasosFuncionamiento.map((p, i) => (
            <div key={p.num} className="flex-1 flex flex-col items-center text-center relative">
              ...
              {i < pasosFuncionamiento.length - 1 && (
                <div className="hidden lg:block absolute top-10 left-full w-8 text-green-300 text-xl z-10 -translate-x-1/2">
                  --→
                </div>
              )}
              ...
              <h3 className="text-sm font-semibold text-gray-800 mb-1">{p.titulo}</h3>
      ...
      <section className="py-14 px-6 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-xl font-bold text-gray-800 text-center mb-10">¿Qué evaluamos?</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6 mb-8">
            {dimensiones.map((d) => (
              <div key={d.titulo} className="bg-white rounded-2xl p-5 text-center shadow-sm">
                <div className="text-4xl mb-3">{d.emoji}</div>
                <h3 className="text-sm font-semibold text-gray-800 mb-1">{d.titulo}</h3>
                <p className="text-xs text-gray-500">{d.desc}</p>
              </div>
            ))}
          </div>
          <div className="bg-white border border-gray-100 rounded-2xl p-4 flex items-center gap-3 text-sm text-gray-500">
      ...
      <section className="py-16 px-6 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <div>
            <h2 className="text-xl font-bold text-gray-800 mb-2">¿Cómo se construye tu perfil?</h2>
            ...
            <div className="space-y-4">
              {barras.map((b) => (
                <div key={b.label} className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-gray-700 w-44">{b.label}</span>
                  ...
                  <span className="text-xs text-gray-500 w-16 text-right">{b.nivel}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="bg-green-50 rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-bold text-gray-800">Perfil único</h3>
            </div>
      ...
      <section className="py-16 px-6 bg-gray-50">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          {/* Qué obtienes */}
          <div>
            <h2 className="text-xl font-bold text-gray-800 mb-6">¿Qué obtienes al finalizar?</h2>
            <ul className="space-y-3">
              {obtienes.map((o) => (
                <li key={o} className="flex items-center gap-3 text-sm text-gray-700">
          ...
          {/* Ejemplo resultado */}
          <div>
            <h2 className="text-xl font-bold text-gray-800 mb-1">Ejemplo de resultado</h2>
            <p className="text-xs text-gray-400 mb-5">Este es un ejemplo de lo que verás al finalizar tu test.</p>
            <div className="grid grid-cols-2 gap-4">
              {/* Fortalezas */}
              <div>
                <p className="text-xs font-semibold text-green-700 mb-3">Fortalezas principales</p>
              ...
              {/* Carreras */}
              <div>
                <p className="text-xs font-semibold text-green-700 mb-3">Carreras recomendadas</p>
                <div className="space-y-2">
                  {carrerasRecomendadas.map((c) => (
                    <div key={c.label} className="flex items-center gap-1.5 text-xs text-gray-700">
                      <span>{c.emoji}</span> {c.label}
                    </div>
                  ))}
                  <p className="text-xs text-green-600 font-medium mt-1">Y más opciones para ti...</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-14 px-6 bg-green-50">
        <div className="max-w-6xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-800 mb-1">¿Listo para descubrir tu camino?</h2>
              <p className="text-sm text-gray-500">
          ...
          <div className="flex gap-3 flex-shrink-0">
            ...
            <button
              onClick={() => navigate('/servicios')}
              className="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-semibold rounded-xl border-2 border-[var(--color-primary)] text-green-700 hover:bg-green-100 transition-all duration-200"
            >
      ...
      <footer className="py-4 px-6 bg-white border-t border-gray-100">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-center gap-6 text-xs text-gray-400">
